...
// Reliable returns whether the network is reliable.
...
// LongReordering returns whether the network reorders long.
...
// LongDelays returns whether the network has long delays.
...
// DeleteServer deletes a server from the network.
...
// GetTotalCount returns the total count from the server.
func (rn *Network) GetTotalCount() int {
	x := atomic.LoadInt32(&rn.count)
	return int(x)
}

// GetTotalBytes returns the total bytes from the server.
...
// GetCount returns the count from the server.
